...
static void bcm2835_peripherals_realize(DeviceState *dev, Error **errp)
{
    ...
    sysbus_connect_irq(SYS_BUS_DEVICE(&s->timer), 0,
        qdev_get_gpio_in_named(DEVICE(&s->ic), BCM2835_IC_ARM_IRQ,
                               INTERRUPT_ARM_TIMER - ARM_IRQ0_BASE)); /* XXX */
    ...
    sysbus_connect_irq(SYS_BUS_DEVICE(&s->dma), 2,
                       qdev_get_gpio_in_named(DEVICE(&s->ic), BCM2835_IC_GPU_IRQ, INTERRUPT_VC_DMA2));
    sysbus_connect_irq(SYS_BUS_DEVICE(&s->dma), 3,
                       qdev_get_gpio_in_named(DEVICE(&s->ic), BCM2835_IC_GPU_IRQ, INTERRUPT_VC_DMA3));
